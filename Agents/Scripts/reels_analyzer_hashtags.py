...
class ReelsHashtagAnalyzer:
    ...
    def stage1_search_reels(self) -> list[dict]:
        ...
        try:
            ...
            # Check for existing CSV as fallback
            print(f"   ❌ Apify error: {e}")
            print("   🔍 Checking for existing data in Executions/...")
            
            latest_csv = None
            for file in EXECUTIONS_DIR.glob("reels_data_*.csv"):
                if not latest_csv or file.stat().st_mtime > latest_csv.stat().st_mtime:
                    latest_csv = file
            
            if latest_csv:
                print(f"   📂 Found existing data: {latest_csv.name}")
                with open(latest_csv, 'r', encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    self.reels_data = list(reader)
                # Convert numeric fields back to int/float
                for r in self.reels_data:
                    for k in ['views', 'likes', 'comments', 'duration']:
                        try:
                            r[k] = int(float(r.get(k, 0)))
                        except:
                            r[k] = 0
                print(f"   ✅ Loaded {len(self.reels_data)} reels from backup")
    ...
    def stage2_transcribe(self) -> None:
        ...
        if not self.whisper_cmd and not self.openai_client:
            print("   ❌ CRITICAL: Neither Whisper nor OpenAI API available!")
            return
        
        print(f"   Model: {self.whisper_model} (or OpenAI API)")
    ...
    def _transcribe_video(self, video_path: str) -> str:
        ...
        # Fallback to OpenAI API if enabled
        if self.openai_client:
            return self._transcribe_with_api(video_path)
            
        return ""

    def _transcribe_with_api(self, video_path: str) -> str:
        ...
        try:
            with open(video_path, "rb") as audio_file:
                transcript = self.openai_client.audio.transcriptions.create(
                    model="whisper-1", 
                    file=audio_file,
                    language="ru" # Force Russian or detect? Typically auto-detect is fine, but for RU reels it helps.
                )
            return transcript.text.strip()
        except Exception as e:
            # print(f"[API Whisper Error: {str(e)[:20]}]", end=" ")
            pass
        return ""
    ...
    # =========================================================================
    # STAGE 6: Propose Topics (GPT)
    # =========================================================================

    def stage6_propose_topics(self, analysis: dict) -> list[dict]:
        """Generate topic proposals based on analysis using GPT."""
        print("\n" + "="*70)
        print("💡 STAGE 6: GENERATING CONTENT IDEAS (GPT)")
        print("="*70)

        if not self.openai_client:
            print("   ⚠️ OpenAI API Key not found. Skipping topic generation.")
            return []

        top_reels = analysis.get("top_10", [])
        if not top_reels:
            return []

        # Prepare context for GPT
        context_reels = ""
        for i, r in enumerate(top_reels[:5]):
            context_reels += f"{i+1}. Hook: {r.get('hook', 'N/A')}\n   Views: {r['views']}\n   Transcipt: {r.get('transcription', '')[:200]}...\n\n"

        prompt = f"""
        You are an expert social media strategist. I have analyzed top performing Instagram Reels for hashtags: {', '.join(self.hashtags)}.
        
        Here are the TOP 5 performing reels:
        {context_reels}

        Based on these successful patterns, generate 3 specific content ideas (Scenarios) for new Reels that would likely go viral.
        
        For each idea provide:
        1. Title (Theme)
        2. The Hook (first 3 seconds text/audio)
        3. Why it works (based on data)
...
"""
        ...
        if analysis.get("proposed_topics"):
             content += "\n## 💡 Предлагаемые сценарии (AI Generated)\n"
             for i, topic in enumerate(analysis["proposed_topics"]):
                 content += f"### {i+1}. {topic['title']}\n"
                 content += f"- **Hook:** \"{topic['hook']}\"\n"
                 content += f"- **Почему сработает:** {topic['rationale']}\n\n"
    ...
    def run_full_pipeline(self) -> dict:
        ...
        # Stage 5 (New Step order): Propose Topics
        topics = self.stage6_propose_topics(analysis)
        analysis["proposed_topics"] = topics
